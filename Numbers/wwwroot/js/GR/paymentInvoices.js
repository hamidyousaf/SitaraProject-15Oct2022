...
function calculateTax(id) {
    debugger;
    var PaymentAmount = parseFloat($('#PaymentAmount_' + id).val().replace(/,/g, ""));
    var Balance = parseFloat($('#Balance_' + id).val().replace(/,/g, ""));
    if (PaymentAmount > Balance) {
        swal("", "Payment Amount Can't by Greate then Invoice Balance", "warning");
        $('#PaymentAmount_' + id).val(0);
        $('#PaymentAmount_' + id).focus();
        return false;
    }
    debugger;
    //tax slab defined
